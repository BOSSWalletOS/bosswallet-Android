...
import static com.bosswallet.app.entity.MagicLinkParcel.generateReverseTradeData;
import static com.bosswallet.token.tools.ParseMagicLink.currencyLink;
import static com.bosswallet.token.tools.ParseMagicLink.customizable;
import static com.bosswallet.token.tools.ParseMagicLink.normal;
import static com.bosswallet.token.tools.ParseMagicLink.spawnable;
import static com.bosswallet.token.tools.ParseMagicLink.unassigned;
...
import com.bosswallet.app.entity.tokens.TokenFactory;
...
public class ImportTokenViewModel extends BaseViewModel implements TransactionSendHandlerInterface
{
    ...
    ImportTokenViewModel(GenericWalletInteract genericWalletInteract,
                         ...
                         BossWalletService bossWalletService,
        ...
        this.bossWalletService = bossWalletService;
    ...
    public void importThroughFeemaster(String url)
    {
        try
        {
            initParser();
            MagicLinkData order = parser.parseUniversalLink(universalImportLink);
            disposable = bossWalletService.handleFeemasterImport(url, wallet.getValue(), network.getValue().chainId, order)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
    ...
    {
        disposable = bossWalletService.checkFeemasterService(feemasterServer, network.getValue().chainId, importOrder.contractAddress)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
